...
if (rows == 4 and colums == 16)fail = -4; /// to fix the library problem with 4*16 display http://forum.arduino.cc/index.php/topic,14604.0.html
 ...
 if (lcdindex > colums-1)
 {
  ...
  {
    lcd.write(128);               //lcd.setCursor(i+fail,rows-2);
    lcd.write(line1[i]);
    lcd.write(148);               //lcd.setCursor(i+fail,rows-1);
    lcd.write(32);
  }
 }
 line1[lcdindex]=asciinumber;
 lcd.write(148+lcdindex);         //lcd.setCursor(lcdindex+fail,rows-1);
...
/////////////////////////////////////
  ...
  if (wpm<10){
    lcd.write(128);               //lcd.setCursor((place)-2,0);
    lcd.print("0");
    lcd.write(129);               //lcd.setCursor((place)-1,0);
    lcd.print(wpm);
    lcd.write(130);               //lcd.setCursor((place),0);
    lcd.print(" WPM");
  }
  else{
    lcd.write(128);                //lcd.setCursor((place)-2,0);
