...
// 06/26/13:  Added Genesis 2" meter values for 150/15 gpm
...
*    650       643           7     
*
*  High/Low Flow Values for Genesis 2" meter (K-factor = 1204.7)
*    GPM    HFlow(fast)  LFlow(slow)
*    150      47            1529    <-  use delayMicroseconds()
***************************************/

const int highFlowValve = 3;  //connect to pin A3 (High Flow Valve Output)
const int lowFlowValve = 4;   //connect to pin A4 (Low Flow Valve Output)
const int pulseOut = 8;       //connect to pin D8 (Pulse Output from Arduino)
const int pulseInd = 13;      //Either hook up an LED to pin13 & GND, or use onboard indicator LED
int valHigh = 0;
int valLow  = 0;
const int fast = 1860;    // frequency value for High Flow rate (see table above)
const int slow = 25;      // frequency value for Low Flow rate (see table above)
